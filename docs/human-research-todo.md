...
- [ ] **Is wizwarr's `FUN_ff69` a save/restore trampoline?** (`grm-mej.3` increment 1, blocks
      ...
      **The question:** does `FUN_ff69` really restore the entry bank before returning — i.e. is the
      ...
      Do **not** bless ironsword or wizwarr until this is answered. Note wizwarr is **also** blocked
      from blessing by `grm-p9y` (the answered `ffa6` question — a confirmed engine bug, awaiting the
      fix, not awaiting an answer), so clearing this item alone does not unblock that golden.
