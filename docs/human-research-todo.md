...
| Is ff1's bistability still real — close `grm-4nr` or keep it as a standing watch item? | **Closed not-reproducible, 2026-08-15 (user ruling).** Filed on 5 runs splitting 3:2 (`refs -33 / instrs -44 / warnings -14`); then 10/10 identical on 2026-08-09/10 and 19/19 on the 2026-08-11 GME sweep, so the original 40% rate is statistically dead (P = 0.6%). Closed **not-reproducible rather than fixed** — no jump-table fix landed in that window, so the likely cause is `grm-izu`/`grm-2dr` changing what sits downstream, not the nondeterminism being removed. **Do not bisect for it; a future ff1 FAIL is the cheap signal and the correct trigger to reopen.** **And do not attach ff1 to the megaman jump-table work** — a triage pass called `grm-g73`/`grm-4nr` a "bistability pair" and that is wrong, as the bead itself already said: different board (MMC1 vs UxROM), an order-of-magnitude larger and *same*-signed delta vs megaman's small *opposite*-signed one with warnings unchanged, and **no ff1 dispatch site has ever been traced** — the shared-shape claim was assumed, never measured. `grm-g73` (megaman) is untouched and stays open; dodge and rcproam flip too and are tracked there | `grm-4nr` **closed**; megaman half stays on `grm-g73`/`grm-eyn` |
