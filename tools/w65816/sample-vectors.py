...
def main() -> int:
    ...
    # The manifest describes WHAT IS IN THE DIRECTORY, not what this invocation happened to
    # write. Deriving it from --opcodes instead was a real defect (caught 2026-09-05 while
    # adding C9): running the script for one opcode rewrote the manifest to claim coverage of
    # that opcode alone, while the other 32 sampled files sat beside it -- silently turning the
    # one artifact whose job is to stop a PARTIAL sample being read as full coverage into a
    # document that understated coverage instead. Scanning the destination cannot drift from
    # reality the way a parameter can.
    present = sorted({f.name.split(".")[0].lower()
                      for f in args.dest.glob("*.json")
                      if len(f.name.split(".")) == 3})
    present_files = sorted(f.name for f in args.dest.glob("*.json"))
    manifest_lines = [
        "# Generated by tools/w65816/sample-vectors.py -- do not hand-edit.",
        "# Regenerate the WHOLE sample against a full clone with:",
        "#     python3 tools/w65816/sample-vectors.py --source <full-clone-dir>",
        "#         --opcodes " + ",".join(present),
        "# (this file lists every opcode present in the directory, not merely the ones the last",
        "#  invocation wrote, so adding one opcode later does not erase the record of the rest)",
        "#",
        ...
        "",
        "# PARTIAL COVERAGE -- READ THIS BEFORE TRUSTING A 'PASS' ROW FOR AN OPCODE NOT LISTED.",
        "# A passing baseline says nothing about the 256 - N opcodes this sample never runs.",
        f"opcodes_covered: {','.join(o.upper() for o in present)}",
        f"opcodes_covered_count: {len(present)} of 256",
        f"file_count: {len(present_files)}",
        f"files_written_this_run: {written}",
        f"cases_written_this_run: {total_cases}",
    ]
